...
TRACK = scale_image(pygame.image.load("imgs/hg.png"), 0.9)

TRACK_BORDER = scale_image(pygame.image.load("imgs/hgH.png"), 0.9)
...
# ShortestPath = ['A', 'B', 'D', 'C', 'E', 'F']
points = {'A': (80, 300), 'B': (393, 132), 'C': (468, 491), 'D': (835, 123),
          'E': (836, 494), 'F': (1140, 296)}
# PATH = [(393, 132), (835, 123), (1098, 296), (836, 494), (468, 491), (90,300)]
PATH = []
for i in path:
...
def draw(win, images, car, game_info, run):
    ...
    if run:
        ...
        path_text = PLAN_FONT.render(
            f"Path to be taken is:: {game_info.path}", 1, (255, 0, 0))
        win.blit(path_text, (20, HEIGHT - path_text.get_height() - 580))

        route_text = PLAN_FONT.render(
            f"The respective route: {game_info.route}", 1, (255, 0, 0))
        win.blit(route_text, (20, HEIGHT - route_text.get_height() - 560))

        trust_text = PLAN_FONT.render(
            f"The average trust value of the route is: {game_info.trust}%", 1, (255, 0, 0))
        win.blit(trust_text, (20, HEIGHT - trust_text.get_height() - 540))

        elapsed_time_text = PLAN_FONT.render(
            f"Time taken to generate the plan: {game_info.elapsed_time}s", 1, (255, 0, 0))
        win.blit(elapsed_time_text, (20, HEIGHT - elapsed_time_text.get_height() - 520))

        htn_plan = PLAN_FONT.render("HTN Plan:", 1, (0, 0, 250))
        win.blit(htn_plan, (20, HEIGHT - htn_plan.get_height() - 500))

        for ii in range(len(plan)):
            plan_text = PLAN_FONT.render(
                f"{game_info.plan[ii]}", 1, (0, 0, 250))
            win.blit(plan_text, (25, HEIGHT - plan_text.get_height() - (500 - ii * 15)))
